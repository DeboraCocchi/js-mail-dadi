  ...
  let mailInserita = userMail.value;
    ...
    console.log(mailInserita);
      ...
      if(isMailInList===true){
        document.getElementById('riscontro-mail').innerHTML='Bentrovato Mario!';
...
lancioDadi.addEventListener('click', function(){
  ...
  if(tiroComputer>tiroUser){
    document.getElementById('vincitore').innerHTML= `Vince il <strong>computer</strong>`;
  }else if(tiroComputer<tiroUser){
    document.getElementById('vincitore').innerHTML= `Vince lo <strong>user</strong>`;
  }else{
